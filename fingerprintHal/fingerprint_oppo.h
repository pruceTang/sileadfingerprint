...
typedef struct fingerprint_device {
...
#ifdef VENDOR_EDIT	
    /*
     * selftest return: 0 on success
     *                  or a negative number in case of failed
	 * fingerprint sensor selftest for factory test.
     */
    int (*selftest)(struct fingerprint_device *dev);	

    /*
     * getImageQuality return: quality value
     */
    int (*getImageQuality)(struct fingerprint_device *dev);
	
    /*
     * fingerprint sensor wait homekey abort for cancel wait homekey.
     */
    int (*homekey_abort)(void);
	
    /*
     * fingerprint wait homekey down
	 * Blocking function,if wait error return false,homekey down return ture.
     */
    int (*waitHomekeyDown)(struct fingerprint_device *dev);
#endif	
    /*
     * enroll pause
     */
    ...
    /*
     * wait finger touch down
     */
    int (*wait_touch_down)(struct fingerprint_device *dev);
    ...
    /*
     * fingerprint touch down msg
     */
    int (*touchDown)(struct fingerprint_device *dev);	
	
    /*
     * fingerprint touch up msg
     */
    int (*touchUp)(struct fingerprint_device *dev);
	
    /*
     * send fingerprint cmd for other functions
     */
...
/*****
    ...
    int (*pauseEnroll)(struct fingerprint_device *dev);
    int (*continueEnroll)(struct fingerprint_device *dev);
    int (*pauseIdentify)(struct fingerprint_device *dev);
    int (*continueIdentify)(struct fingerprint_device *dev);
    int (*getAlikeyStatus)(struct fingerprint_device *dev);
    ...
    int (*dynamicallyConfigLog)(struct fingerprint_device *dev, uint32_t on);
