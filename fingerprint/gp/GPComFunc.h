...
typedef struct gp_handle {
    ...
    int32_t (*TEECCom_load_trustlet)(struct gp_handle* gp_handle, TEEC_Session **clnt_handle, const char *path, const TEEC_UUID tauuid);
    void (*TEECCom_close_trustlet)(struct gp_handle* gp_handle);
} gp_handle_t;

int gp_open_handle(gp_handle_t **handle);
int gp_free_handle(gp_handle_t **handle);
