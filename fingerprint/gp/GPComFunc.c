...
//#define GP_LIBRARY "libTeeClient.so"
#define GP_LIBRARY "libMcClient.so"

static TEEC_Context  *m_context;
static TEEC_Session  *m_session;

typedef struct {
    void *libHandle;
} _priv_data_t;

static void gp_close(gp_handle_t* gp_handle);
static int gp_load_trustlet(gp_handle_t* gp_handle, TEEC_Session **clnt_handle,
                            const char *path, const TEEC_UUID tauuid);
...
int gp_free_handle(gp_handle_t** handle_ptr)
{
    _priv_data_t *data = NULL;
    gp_handle_t *handle = NULL;
...
static TEEC_Result _gp_initialize(gp_handle_t* gp_handle, TEEC_Context **context)
{
    ...
    *context = NULL;
    ...
    LOG_MSG_VERBOSE("%s finished", __func__);
    return nError;
}
...
{
    if (context == NULL) {
        LOG_MSG_DEBUG("Device handle invalid");
        return TEEC_ERROR_BAD_PARAMETERS;
    }
...
{
    if (session == NULL) {
        LOG_MSG_DEBUG("Invalid session handle");
        return TEEC_ERROR_BAD_PARAMETERS;
    }
...
static TEEC_Result _gp_open_session(gp_handle_t* gp_handle, TEEC_Context *context, TEEC_Session **session, const TEEC_UUID *tauuid)
{
    ...
    *session = (TEEC_Session *)malloc(sizeof(TEEC_Session));
    ...
    if (nError != TEEC_SUCCESS) {
        LOG_MSG_DEBUG("TEEC_OpenSession 0x%X failed (%08x), %s", uuid.timeLow, nError, gp_error_strings(nError));
        free(*session);
        return nError;
    }
    return TEEC_SUCCESS;
...
static int gp_load_trustlet(gp_handle_t* gp_handle, TEEC_Session **clnt_handle, const char __unused *path, const TEEC_UUID tauuid)
{
    TEEC_Result nError = TEEC_SUCCESS;

    nError = _gp_initialize(gp_handle, &m_context);
    if (nError != TEEC_SUCCESS) {
        LOG_MSG_DEBUG("_gp_initialize failed (%08x), %s", nError, gp_error_strings(nError));
        return -1;
    }

    /* Open a session */
    nError = _gp_open_session(gp_handle, m_context, &m_session, &tauuid);
    if (nError != TEEC_SUCCESS) {
        LOG_MSG_DEBUG("_gp_open_session failed (%08x), %s", nError, gp_error_strings(nError));
        return -1;
    } else if (clnt_handle) {
        *clnt_handle = m_session;
    }
...
static void gp_close(gp_handle_t* gp_handle)
{
    ...
    /* Close the session */
    nError = _gp_close_session(gp_handle, m_session);
    if (nError != TEEC_SUCCESS) {
    ...
    /* Finalize */
    nError = _gp_finalize(gp_handle, m_context);
    if (nError != TEEC_SUCCESS) {
