...
static uint64_t m_imaq_total_time = 0;
static uint32_t m_imaq_total_count = 0;
static uint32_t m_imaq_current_time = 0;
...
{
    if (match) {
        LOG_MSG_INFO("img[(%d:%d:%d)] match[(%d:%d:%d) (%d:%d:%d)]",
                     m_imaq_current_time, m_imaq_total_count, (uint32_t)(m_imaq_total_time/m_imaq_total_count),
        ...
        LOG_MSG_INFO("img[(%d:%d:%d)] mis[(%d:%d:%d) (%d:%d:%d)]",
                     m_imaq_current_time, m_imaq_total_count, (uint32_t)(m_imaq_total_time/m_imaq_total_count),
...
void silfp_stats_reset(void)
{
    if (m_stats_enabled) {
        ...
        m_imaq_total_time = 0;
        m_imaq_total_count = 0;
        m_imaq_current_time = 0;
...
void silfp_stats_capture_image()
{
    if (m_stats_enabled) {
        ...
        if (end >= m_begin_time) {
            m_imaq_current_time = (uint32_t)(end - m_begin_time);
            m_imaq_total_count++;
            if (m_imaq_total_count == 0) {
                m_imaq_total_count++;
                m_imaq_total_time = m_imaq_current_time;
            } else {
                m_imaq_total_time += m_imaq_current_time;
            }
        }
