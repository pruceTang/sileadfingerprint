...
#include "silead_fingerext_event.h"
#include "silead_fingerext_hub.h"

#define MAX_COMMAND_BYTES (48 * 1024)
...
static void _ext_hub_process_wakeup_callback(int __unused fd, short __unused flags, void __unused *param)
{
    char buff[16];
    int32_t ret;
...
static void _ext_hub_listen_callback(int fd, short __unused flags, void __unused *param)
{
    int32_t ret;
...
static void _ext_hub_process_command_callback(int fd, short __unused flags, void *param)
{
    void *p_record;
    size_t recordlen;
    int32_t ret;
    ...
    for (;;) {
        ...
        if (ret == 0 && p_record == NULL) {
            ...
            silfp_ext_test_cmd(p_record, recordlen);
        }
    }
...
static int32_t _ext_hub_data_blocking_write(int fd, const void *buffer, size_t len)
{
    size_t offset = 0;
...
int32_t silfp_ext_hub_send_response_raw(uint32_t cmdid, const void *data, size_t data_size)
{
    ...
    if (data_size + sizeof(uint32_t) > MAX_COMMAND_BYTES) {
        LOG_MSG_ERROR("packet larger than %u (%u)", MAX_COMMAND_BYTES, (unsigned int )data_size);
        return -1;
    }
